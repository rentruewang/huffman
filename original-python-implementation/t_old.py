'''
The first working model of this project,
(probably beating old.py, which was refined later.)

The information above is probably irrelevant, though.

If you want a plain python file,
that does all the encode, decode, saveing job,
this is the one for you.

If you also want to converter pdf to txt files,
use old.py instead. 
'''
...
# text = textract.process('gone with the wind.pdf')
...
counter = 0
with open(txt) as file,open(r'./'+newpath+r'/'+c_file, 'w+') as cached:
    for line in file:
        for ch in line:
...
# 
# 
enc = txt.replace('.txt','')+'_'+'encoded'


with open(enc, 'wb+') as encoded,open(r'./'+newpath+r'/'+c_file, 'r') as cached:
    ft = ''
    while True:
        ft = cached.read(8)
        if(len(ft)==0):
...
result = enc.replace('encoded','decoded')+'.txt'
with open(enc,'rb') as binary,open(r'./'+newpath+r'/'+d_cache,'w+') as wr:
        ft = ''
        length = 1
        while True:
            ft = binary.read(1)
            ...
            wr.write(ft)
def decode():
    with open(r'./'+newpath+r'/'+d_cache,'r') as cache,open(result,'w+') as file:
...
decode()
print('the reason i create those cached files is that they reduce the efficiency of reading the file from O(n^2) down to O(n)')
